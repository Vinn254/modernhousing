...
export default function AgentTenantsPage() {
    ...
    async function loadData() {
        ...
        try {
            ...
            setUnits((unitsResult.units ?? []).filter((u) => u.property_id === storedPropertyId));
        }
        catch (err) {
    ...
    async function handleMarkRelocated(unitId) {
        if (!confirm('Mark this tenant as relocated? This will free the unit.'))
            return;
        const response = await fetch('/api/units', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
    ...
    async function handleViewBills(tenant) {
    ...
    return (<main className="container admin-no-hero">
          ...
          <form onSubmit={handleAddTenant} className="form-grid">
            ...
            <div className="field-group">
              ...
              <select value={form.unitNumber} onChange={e => setForm(f => ({ ...f, unitNumber: e.target.value }))} required>
                ...
                {units.filter(u => u.occupancy_status === 'vacant').map(u => (<option key={u.id} value={u.unit_number}>{u.unit_number} ({u.unit_type || 'unit'})</option>))}
              </select>
            </div>
            <div className="field-group">
      ...
      <section className="card-grid-item" style={{ marginTop: 24 }}>
        <article className="card">
          ...
          {!loading && tenants.length > 0 && (<div className="table-shell">
                ...
                return (<tr key={tenant.id}>
                        ...
                        <td>
                          <div className="landlord-actions">
                            <button type="button" className="action-button" style={{ padding: '6px 12px', fontSize: '12px' }} onClick={() => unit && handleMarkRelocated(unit.id)}>
                              Mark Relocated
                            </button>
                            <button type="button" className="action-button primary" style={{ padding: '6px 12px', fontSize: '12px', marginLeft: 8 }} onClick={() => handleViewBills(tenant)}>
                              View Bills
                            </button>
                          </div>
                        </td>
      ...
      {selectedTenant && (<section className="card-grid-item" style={{ marginTop: 24 }} ref={formRef}>
          <article className="card" style={{ gridColumn: 'span 2' }}>
            ...
            {showDirectPayment && (<div className="card" style={{ marginBottom: 16, background: 'var(--surface)' }}>
            ...
            {!loadingBills && bills.length > 0 && (<div className="table-shell" style={{ maxHeight: '400px', overflowY: 'auto' }}>
                <table className="landlord-table" style={{ fontSize: '12px' }}>
                  ...
                  <tbody>
                    {bills.map(bill => (<tr key={bill.id}>
                        ...
                        <td>
                          {bill.balance > 0 && (<button className="action-button warn" style={{ padding: '6px 12px', fontSize: '12px' }} onClick={() => handleShowPayForm(bill.id, bill.balance)}>Pay</button>)}
                        </td>
                      </tr>))}
                  </tbody>
                </table>
