...
export default function TenantPaymentsPage() {
    ...
    async function loadPayments(email, tenantId = null) {
        ...
        if (billsResponse?.ok) {
            ...
            allBills = (billsResult.bills ?? []).map((b) => ({
                id: b.id,
        ...
        if (paymentsResponse?.ok) {
            ...
            const legacyBills = (paymentsResult.payments ?? []).map((p) => ({
                id: p.id,
    ...
    async function handleStkPush(event) {
        ...
        setProcessing(false);
        if (!response.ok) {
            setError(result.message ?? 'Payment failed - check landlord Daraja settings in Payment Settings');
            return;
        }
    ...
    const calculateWithRunningBalance = (billsList) => {
    ...
    function drawTableFullRow(page, font, bill, y) {
    ...
    return (<main className="container page-layout">
      ...
      {user && (<section className="card-grid" style={{ marginBottom: 24 }}>
          <article className="card">
            ...
            {activeTab === 'payments' && (<form onSubmit={handleStkPush} className="form-grid">
                <select value={paymentType} onChange={e => setPaymentType(e.target.value)} style={{ marginBottom: 8 }}>
            ...
            {activeTab === 'utilities' && (<form onSubmit={handleStkPush} className="form-grid">
                <input type="tel" value={mpesaPhone} onChange={e => setMpesaPhone(e.target.value)} required placeholder="M-Pesa Phone (07XX XXX XXX)"/>
                <input type="number" value={mpesaAmount} onChange={e => setMpesaAmount(e.target.value)} required placeholder="Amount (KES)" min="1"/>
                <button type="submit" disabled={processing} className="action-button primary">{processing ? 'Processing…' : 'Pay Now'}</button>
              </form>)}
      ...
      <section className="card" style={{ marginTop: 24 }}>
        ...
        {activeTab === 'invoices' ? (invoices.length === 0 ? (<p className="landlord-empty">No invoices available.</p>) : (<div className="table-shell" style={{ maxHeight: '500px', overflowY: 'auto' }}>
              <table className="landlord-table" style={{ fontSize: '12px' }}>
                ...
                return aOrder - bOrder || (a.month_due || '').localeCompare(b.month_due || '');
                      ...
                      <td>
                        {inv.file_path ? (<a href={inv.file_path} target="_blank" rel="noopener noreferrer" className="action-button secondary" style={{ padding: '6px 12px', fontSize: '12px' }}>Download</a>) : (<span style={{ color: '#9ca3af', fontSize: '11px' }}>Not generated</span>)}
                      </td>
                    </tr>))}
                </tbody>
              </table>
            </div>)) : ((activeTab === 'payments' ? rentWithBalance : utilityWithBalance).length === 0 ? (<p className="landlord-empty">{activeTab === 'payments' ? 'No rent payments recorded yet.' : 'No utility bills recorded yet.'}</p>) : (<div className="table-shell" style={{ maxHeight: '500px', overflowY: 'auto' }}>
              <table className="landlord-table" style={{ fontSize: '12px' }}>
                <thead>
                  <tr>
                    <th>Month</th>
                ...
                <tbody>
                  {(activeTab === 'payments' ? rentWithBalance : utilityWithBalance).map(bill => (<tr key={bill.id}>
