...
export async function GET(request) {
    try {
        ...
        if (landlordProfilesResult.error)
            throw landlordProfilesResult.error;
        const profiles = (profilesResult.data ?? []);
        const landlordProfiles = (landlordProfilesResult.data ?? []);
        const landlordById = new Map(landlordProfiles.map((profile) => [profile.user_id, profile]));
        const usersById = new Map(users.map((user) => [user.id, user]));
        let agents = profiles.map((profile) => {
        ...
        if (landlordId) {
            agents = agents.filter((agent) => agent.landlord_email === landlordId || agent.property_id === landlordId);
        }
