...
export async function GET(request) {
    ...
    let query = supabaseAdmin.from('audit_trails').select(`
    *,
...
export async function POST(request) {
    ...
    const { documentId, tenantId, tenantName, tenantEmail, tenantNationalId, signatureType, event, ipAddress, device, disclosureConsent } = body;
    const authContext = await getAuthContext(request);
    ...
    if (existingTrail) {
        ...
        if (event === 'sent' && !existingTrail.sent_at)
            updateData.sent_at = now;
        if (event === 'viewed' && !existingTrail.viewed_at)
            updateData.viewed_at = now;
        if (event === 'signed' && !existingTrail.signed_at)
            updateData.signed_at = now;
        if (event === 'completed' && !existingTrail.completed_at)
        ...
        if (event === 'sent')
            createData.sent_at = now;
        if (event === 'viewed')
            createData.viewed_at = now;
        if (event === 'signed')
            createData.signed_at = now;
        if (event === 'completed')
            createData.completed_at = now;
        if (disclosureConsent)
            createData.disclosure_consent = disclosureConsent;
        if (event === 'consent_accepted')
            createData.consent_accepted_at = now;
        const { data, error } = await supabaseAdmin
            .from('audit_trails')
            .insert(createData)
            .select()
