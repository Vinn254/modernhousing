...
export async function POST(request) {
    try {
        ...
        if (error) {
            ...
            return NextResponse.json({ message: 'Unable to send reset link. Please try again later.' }, { status: 500 });
        }
        if (supabaseAdmin) {
            try {
                ...
                if (isSuperAdmin) {
                    return NextResponse.json({ message: 'Password reset is not available for the super admin account.' }, { status: 403 });
                }
            }
            catch (adminError) {
