...
export default function AgentUtilitiesPage() {
    ...
    function calculateWaterBill(consumption) {
        if (consumption <= 0)
            return 0;
        if (consumption <= 6)
            return consumption * 88;
        if (consumption <= 20)
            return consumption * 132;
        if (consumption <= 50)
            return consumption * 137;
        if (consumption <= 100)
            return consumption * 148;
        if (consumption <= 300)
    ...
    async function loadData() {
        ...
        try {
            ...
            if (invoicesResponse.ok)
                setPayments((invoicesResult.invoices ?? []).filter((i) => utilityTypes.includes(i.invoice_type)));
            if (unitsResponse.ok)
        ...
        <div className="bento">
          <article className="card">
            ...
            {units.length === 0 ? (<p style={{ color: 'var(--ink-3)', fontSize: '13px' }}>No units available.</p>) : (<div style={{ maxHeight: '240px', overflow: 'auto', marginBottom: 12, border: '1px solid var(--line)', borderRadius: '8px', padding: '8px' }}>
                {units.map((unit) => (<div key={unit.id} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--line-soft)' }}>
          ...
          <article className="card" style={{ gridColumn: 'span 2' }}>
            ...
            {!loading && payments.filter(p => utilityTypes.includes(p.invoice_type)).length > 0 && (<div className="table-shell">
                <table className="landlord-table">
                  <thead><tr><th>Tenant</th><th>Utility</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead>
                  <tbody>
                    {payments.filter(p => utilityTypes.includes(p.invoice_type)).map(payment => (<tr key={payment.id}><td className="landlord-name">{payment.tenants?.full_name ?? '—'}</td><td>{payment.invoice_type}</td><td>{formatCurrency(payment.amount)}</td><td>{payment.status}</td><td>{payment.created_at ? new Date(payment.created_at).toLocaleDateString() : ''}</td></tr>))}
                  </tbody>
                </table>
              </div>)}
          </article>
