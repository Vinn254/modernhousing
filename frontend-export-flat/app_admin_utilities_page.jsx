...
export default function UtilitiesPage() {
    ...
    const [waterMeterReadings, setWaterMeterReadings] = useState({});
    ...
    async function loadData() {
        ...
        try {
            ...
            const utilityBills = (billsResult.bills ?? []).filter((b) => ['garbage', 'service_charge', 'parking', 'security', 'internet', 'laundry', 'pet_fees', 'other'].includes(b.transaction_type));
    ...
    async function handleAddUtility(event) {
    ...
    async function handleAddWaterBill(event) {
        event.preventDefault();
        setMessage('');
        setError('');
    ...
    async function handleWaterMeterReading(unitId) {
    ...
    async function handleShowPayForm(billId, balance) {
    ...
    async function handleShowEditForm(bill) {
        setEditForm({
            billId: bill.id,
    ...
    async function handleUpdateBill(event) {
        event.preventDefault();
        setMessage('');
        setError('');
        const response = await fetch('/api/bills', {
      ...
      <main className="container admin-no-hero">
        ...
        <section className="card-grid">
          ...
          <article className="card">
            ...
            {units.length === 0 ? (<p style={{ color: 'var(--ink-3)', fontSize: '13px' }}>No units available.</p>) : (<div style={{ maxHeight: '240px', overflow: 'auto', marginBottom: 12, border: '1px solid var(--line)', borderRadius: '8px', padding: '8px' }}>
                {units.map((unit) => {
                const consumption = Number(waterMeterReadings[unit.id] || 0);
                const billAmount = calculateWaterBill(consumption);
                return (<div key={unit.id} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--line-soft)' }}>
        ...
        {showEditForm && (<div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <section className="card" style={{ marginTop: 24 }}>
          {showPayForm && (<div className="card" style={{ marginBottom: 16, background: 'var(--surface)' }}>
              <h3>Make Payment</h3>
          ...
          {!loading && bills.length > 0 && (<div className="table-shell" style={{ maxHeight: '500px', overflowY: 'auto' }}>
              <table className="landlord-table" style={{ fontSize: '12px' }}>
                ...
                <tbody>
                  {bills.map(bill => (<tr key={bill.id}>
                      ...
                      <td>
                        {bill.balance > 0 && (<button className="action-button primary" style={{ padding: '4px 8px', fontSize: '11px', marginRight: 4 }} onClick={() => handleShowPayForm(bill.id, bill.balance)}>Pay</button>)}
