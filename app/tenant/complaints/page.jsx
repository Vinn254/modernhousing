...
export default function TenantCommunicationComplaintsPage() {
    ...
    function buildMessageItem(item, fallbackSelectedId) {
        return {
            ...item,
    ...
    async function loadContext(userId, email) {
        try {
    ...
    async function handleDeleteNotification(messageId) {
    ...
    async function handleSendReply(messageId) {
    ...
    function handleKeyDown(event, messageId) {
        ...
        <section className="card-grid" style={{ marginTop: 32, display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', alignItems: 'stretch', gap: 20, width: '100%' }}>
          <article className="card" style={{ minHeight: 260, display: 'flex', flexDirection: 'column', background: 'linear-gradient(135deg, #f0fdfa 0%, #ccfbf1 100%)' }}>
            ...
            <form onSubmit={handleSendDirect} className="form-grid">
              ...
              <textarea value={directForm.messageText} onChange={(event) => setDirectForm((current) => ({ ...current, messageText: event.target.value }))} required placeholder="Write a message to your landlord..." rows={4}/>
          ...
          <article className="card" style={{ minHeight: 260, display: 'flex', flexDirection: 'column', background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)' }}>
            ...
            <form onSubmit={handleComplaint} className="form-grid">
              ...
              <select value={complaintForm.attachmentType} onChange={(event) => setComplaintForm((current) => ({ ...current, attachmentType: event.target.value }))}>
              ...
              <textarea value={complaintForm.messageText} onChange={(event) => setComplaintForm((current) => ({ ...current, messageText: event.target.value }))} required placeholder="Describe the issue in detail, including where it is and how urgent it feels..." rows={3}/>
              <button type="submit" disabled={sending}>{sending ? 'Sending…' : 'Submit Complaint'}</button>
            </form>
            {complaints.length > 0 && (<div className="table-shell" style={{ overflowY: 'auto', flex: 1, maxHeight: 140, marginTop: 12 }}>
          ...
          <article className="card" style={{ minHeight: 260, display: 'flex', flexDirection: 'column', background: 'linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%)' }}>
            ...
            {alertMessages.length === 0 ? (<p className="landlord-muted">No overdue or reminder alerts right now.</p>) : (<div className="table-shell" style={{ overflowY: 'auto', flex: 1, maxHeight: 180 }}>
                <table className="landlord-table">
                  <thead>
                    <tr>
                      <th>Alert</th>
                  ...
                  <tbody>
                    {alertMessages.map((item) => (<tr key={item.id} style={{ cursor: 'pointer' }} onClick={() => {
          ...
          <div className="communications-layout" style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 360px) 1fr', minHeight: 520 }}>
            <aside className="communications-sidebar communications-scroll" style={{ borderRight: '1px solid var(--line)', background: '#f9fafb', maxHeight: 560, overflowY: 'auto' }}>
              ...
              {!loading && messages.length > 0 && messages.map((item) => {
                ...
                return (<div key={item.id} style={{ position: 'relative', borderBottom: '1px solid #e5e7eb', background: isSelected ? '#eefdf3' : 'transparent' }}>
                  ...
                  <button type="button" onClick={() => handleDeleteNotification(item.id)} title="Delete message" style={{ position: 'absolute', right: 10, top: 12, border: '1px solid #fecaca', borderRadius: 999, width: 24, height: 24, background: '#fff1f2', color: '#b91c1c', cursor: 'pointer', fontSize: 13, lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                    ×
                  </button>
                </div>);
            })}
            ...
            <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 16 }}>
              {selectedMessage ? (<>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {selectedMessage.thread.map((entry) => (<div key={entry.id} style={{ padding: 12, borderRadius: 12, background: entry.role === 'You' ? '#ecfdf5' : '#ffffff', border: entry.role === 'You' ? '1px solid #a7f3d0' : '1px solid #e5e7eb' }}>
                  ...
                  <div style={{ padding: 12, border: '1px solid #d1fae5', borderRadius: 12, background: '#f0fdf4' }}>
                    ...
                    <textarea value={replyDrafts[selectedMessage.id] || ''} onChange={(event) => setReplyDrafts((current) => ({ ...current, [selectedMessage.id]: event.target.value }))} onKeyDown={(event) => handleKeyDown(event, selectedMessage.id)} placeholder="Reply with a quick follow-up…" rows={4} style={{ width: '100%', borderRadius: 10, border: '1px solid #a7f3d0', padding: '10px 12px', resize: 'vertical' }}/>
