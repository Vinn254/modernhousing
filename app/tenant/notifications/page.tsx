    ...
    <>
      <style jsx global>{`
        @media (max-width: 900px) {
          .tenant-notifications-shell {
            grid-template-columns: 1fr !important;
          }
          .tenant-notifications-sidebar {
      ...
      <main className="container page-layout">
        ...
        <section className="card" style={{ padding: 0, overflow: 'hidden', border: '1px solid rgba(16, 185, 129, 0.18)', boxShadow: '0 0 20px rgba(16, 185, 129, 0.08)' }}>
          ...
          <div className="tenant-notifications-shell" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 320px) minmax(0, 1fr)', minHeight: 460, gap: 0 }}>
            <aside className="tenant-notifications-sidebar" style={{ borderRight: '1px solid var(--line)', background: '#f9fafb', maxHeight: '60vh', overflowY: 'auto' }}>
              ...
              {!loading && notifications.length > 0 && notifications.map((item) => {
                ...
                return (
                  <button
                    ...
                    style={{ width: '100%', textAlign: 'left', border: 'none', borderBottom: '1px solid #e5e7eb', padding: '12px 14px', background: isSelected ? '#eefdf3' : 'transparent', cursor: 'pointer' }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <strong style={{ fontSize: '12px' }}>{contactLabel}</strong>
                      <span style={{ fontSize: '11px', color: 'var(--ink-3)' }}>{new Date(item.created_at).toLocaleDateString()}</span>
                    </div>
                    <div style={{ marginTop: 6, fontSize: '12px', color: '#111827' }}>{item.message.slice(0, 60)}{item.message.length > 60 ? '…' : ''}</div>
                  </button>
                );
              })}
            </aside>
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ padding: 12, borderRadius: 12, border: '1px solid #d1fae5', background: '#f0fdf4' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                  <button type="button" onClick={handleSendNewMessage} disabled={sending} style={{ padding: '8px 14px', borderRadius: 999, border: 'none', background: '#10b981', color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: '13px' }}>
              ...
              {selectedMessage ? (
                <>
                  ...
                  <div style={{ padding: 12, borderRadius: 12, border: '1px solid #d1fae5', background: '#f0fdf4' }}>
                    ...
                    />
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                      <button type="button" onClick={handleSendReply} disabled={sending} style={{ padding: '8px 14px', borderRadius: 999, border: 'none', background: '#10b981', color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: '13px' }}>
