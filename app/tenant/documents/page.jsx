...
export default function TenantDocumentsPage() {
    ...
    async function loadDocuments() {
        ...
        if (agreementsResponse?.ok)
            setAgreements(agreementsResult.documents ?? []);
        if (docsResponse?.ok)
    ...
    async function handleUploadAll(event) {
        ...
        if (session?.access_token)
            headers.Authorization = `Bearer ${session.access_token}`;
        const uploadFile = async (file, type, name) => {
            if (!file)
                return;
            ...
            if (!response.ok)
                throw new Error(result.message ?? `Failed to upload ${name}`);
        };
        try {
            ...
            if (passportPhoto && tenantId) {
                ...
                const result = await response.json();
                if (!response.ok)
                    throw new Error(result.message ?? 'Failed to upload Passport Photo');
            }
            ...
            if (nextOfKinId && tenantId) {
                ...
                const result = await response.json();
                if (!response.ok)
                    throw new Error(result.message ?? 'Failed to upload Next of Kin ID');
            }
    ...
    const FileInput = ({ label, accept, onChange, file }) => (<div style={{ marginBottom: 12 }}>
    ...
    return (<main className="container page-layout" style={{ maxWidth: '100%', padding: '0 8px' }}>
      ...
      {agreements.filter(d => d.document_type === 'agreement').length > 0 && (<section style={{ marginTop: 24 }}>
          <div className="card">
      ...
      {agreements.length === 0 && (<section style={{ marginTop: 24 }}>
          <div className="card">
      ...
      <section className="card-grid">
        <article className="card" style={{ gridColumn: '1 / -1' }}>
          ...
          <form onSubmit={handleUploadAll} className="form-grid">
            <FileInput label="Signed Agreement (PDF/Image)" accept=".pdf,.jpg,.jpeg,.png" onChange={e => setSignedAgreement(e.target.files?.[0] ?? null)} file={signedAgreement}/>
            <FileInput label="ID Document (PDF/Image)" accept=".pdf,.jpg,.jpeg,.png" onChange={e => setIdDocument(e.target.files?.[0] ?? null)} file={idDocument}/>
    <FileInput label="Passport Photo (JPG/PNG)" accept=".jpg,.jpeg,.png" onChange={e => setPassportPhoto(e.target.files?.[0] ?? null)} file={passportPhoto}/>
             <FileInput label="Next of Kin ID (PDF/Image)" accept=".pdf,.jpg,.jpeg,.png" onChange={e => setNextOfKinId(e.target.files?.[0] ?? null)} file={nextOfKinId}/>
      ...
      {(() => {
            ...
            return (<section className="card" style={{ marginTop: 24, overflowX: 'auto' }}>
            ...
            {(documents.length > 0 || signedAgreementDocs.length > 0 || idDocs.length > 0) && (<div className="table-shell" style={{ overflowX: 'auto' }}>
                <table className="landlord-table" style={{ minWidth: '400px', fontSize: '12px' }}>
                  ...
                  <tbody>
                    {signedAgreementDocs.map(doc => (<tr key={doc.id}>
                    ...
                    {idDocs.some(d => d.document_name !== 'Passport Photo') && (<tr>
                        <td>ID Document</td>
                        <td>
                          <span className={`status-pill ${bundleStatus === 'approved' ? 'status-active' : bundleStatus === 'rejected' ? 'status-pending' : 'status-pending'}`}>
                            {bundleStatus === 'approved' ? 'Approved' : bundleStatus === 'rejected' ? 'Rejected' : 'Pending Review'}
                    ...
                    {idDocs.filter(d => d.document_name === 'Passport Photo').length > 0 && (<tr>
