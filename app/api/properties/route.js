...
async function enrichProperties(rows) {
    if (rows.length === 0)
        return [];
    ...
    const [{ data: units, error: unitsError }, { data: tenants, error: tenantsError }] = await Promise.all([
        ...
        supabaseAdmin
            .from('tenants')
            .select('id, unit_id')
            .in('unit_id', (await supabaseAdmin.from('units').select('id').in('property_id', propertyIds)).data?.map((unit) => unit.id) ?? []),
    ]);
...
async function assertPropertyAccess(propertyId, authContext) {
...
export async function POST(request) {
    ...
    if (!authContext.isSuperAdmin) {
        ...
        // Log audit
        await logAuditEvent(authContext.userId, authContext.userEmail, 'create', 'property', result.data?.id, { name, address, unitCount, amenities });
...
export async function PATCH(request) {
    try {
        ...
        // Log audit
        await logAuditEvent(authContext.userId, authContext.userEmail, 'update', 'property', propertyId, { name, address, amenities });
        const enriched = await enrichProperties([result.data]);
...
export async function DELETE(request) {
    try {
        ...
        // Log audit
        await logAuditEvent(authContext.userId, authContext.userEmail, 'delete', 'property', propertyId, { propertyId });
