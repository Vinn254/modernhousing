        ...
        if (isAgent && agentPropertyId) {
            ...
            if (unitIds.length > 0) {
                const { data: tenantIds } = await client
                    ...
                    .select('id')
                    .in('unit_id', unitIds);
                const tenantIdValues = (tenantIds ?? []).map((t) => t.id);
...
export async function POST(request) {
    ...
    if (contentType.includes('multipart/form-data')) {
        try {
            ...
            const documentType = formData.get('documentType') || 'agreement';
            const documentName = formData.get('documentName') || documentType;
            if (!file || !tenantId) {
            ...
            if (!authContext.isSuperAdmin && authContext.organizationId) {
                ...
                const { data: tenantCheck } = await client
                    ...
                    .eq('id', tenantId)
                    .in('unit_id', (await client.from('units').select('id').in('property_id', propIds)).data?.map((u) => u.id) || []);
            ...
            if (!isLandlordUpload) {
                ...
                if (documentType === 'signed_agreement')
                    bundleUpdate.signed_agreement_url = publicUrl.publicUrl;
                if (documentType === 'id_document' && documentName === 'Passport Photo')
                    bundleUpdate.passport_photo_url = publicUrl.publicUrl;
                if (documentType === 'id_document' && documentName !== 'Passport Photo')
                    bundleUpdate.id_document_url = publicUrl.publicUrl;
                if (documentType === 'kin_id')
                    bundleUpdate.next_of_kin_id_url = publicUrl.publicUrl;
    ...
    const { tenantId, documentName, documentUrl, documentType, notes } = body;
