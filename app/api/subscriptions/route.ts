...
export async function GET() {
  try {
    ...
    if (error) {
      if (isMissingTableError(error, 'subscriptions')) return NextResponse.json({ subscriptions: [] });
      throw error;
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!['paid', 'pending', 'overdue', 'expired', 'active'].includes(status)) return badRequest('Invalid subscription status.');
