...
export default function AdminManagementPage() {
  ...
  async function handleApprove(admin: Admin) {
    const response = await fetch('/api/admins', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDeactivate(admin: Admin) {
    if (!confirm(`Deactivate ${admin.full_name}?`)) return;
  ...
  return (
    <>
      ...
      {selectedAdmin && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setSelectedAdmin(null)}>
