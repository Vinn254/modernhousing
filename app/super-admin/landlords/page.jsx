...
function getSubscriptionForLandlord(landlord, subscriptions) {
...
function generateNotificationMessage(subscription, landlord) {
...
export default function LandlordManagementPage() {
    ...
    useEffect(() => {
        if (selectedLandlord || selectedSubscription || landlords.length === 0 || subscriptions.length === 0)
            return;
    ...
    async function handleAddLandlord(event) {
        ...
        if (result.landlord?.id) {
            const planAmounts = { monthly: 2500, quarterly: 5000, yearly: 6000 };
    ...
    async function handleApprove(landlord) {
        const response = await fetch('/api/landlords', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
    ...
    async function handleResetPassword(landlord) {
    ...
    async function handleDeactivate(landlord) {
        if (!confirm(`Deactivate ${landlord.full_name}? They will not be able to access landlord pages until reactivated. Their subscription must be renewed to be reactivated.`))
            return;
        const response = await fetch('/api/landlords', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
    ...
    async function handleReactivate(landlord) {
        if (!confirm(`Reactivate ${landlord.full_name}? They will regain access to landlord pages.`))
            return;
        const response = await fetch('/api/landlords', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
    ...
    async function handleUpgradeSubscription(landlord) {
    ...
    function openNotification(subscription, landlord) {
        setSelectedSubscription(subscription);
        setSelectedLandlord(landlord);
        setNotificationText(generateNotificationMessage(subscription, landlord));
        setError('');
    }
    function handleComposerLandlordChange(event) {
    ...
    function handleComposerSubscriptionChange(event) {
    ...
    return (<>
            ...
            return (<tr key={landlord.id} className={`risk-row ${risk}`}>
                          ...
                          <td>
                            <div className="landlord-actions">
                              ...
                              {landlord.status === 'active' && <button className="action-button danger" style={{ padding: '6px 10px', fontSize: '12px' }} onClick={() => handleDeactivate(landlord)}>Deactivate</button>}
                              {landlord.status === 'inactive' && landlord.approval_status === 'deactivated' && <button className="action-button primary" style={{ padding: '6px 10px', fontSize: '12px' }} onClick={() => handleReactivate(landlord)}>Reactivate</button>}
      ...
      {selectedLandlord && !selectedSubscription && (<div className="modal-overlay" onClick={(event) => event.target === event.currentTarget && setSelectedLandlord(null)}>
          <div className="modal-card landlord-modal">
            <div className="modal-title-row">
      ...
      {selectedLandlord && selectedSubscription && (<div className="modal-overlay" onClick={(event) => event.target === event.currentTarget && (setSelectedLandlord(null), setSelectedSubscription(null))}>
          <div className="modal-card landlord-modal">
            <div className="modal-title-row">
            ...
            <div className="notification-composer">
              ...
              <div className="composer-controls">
                ...
                <select value={selectedSubscription.id} onChange={handleComposerSubscriptionChange}>
                  {composerSubscriptions.filter((s) => s.admin_id === selectedLandlord.id).map((subscription) => (<option key={subscription.id} value={subscription.id}>{subscription.plan} - {new Date(subscription.expiry_date).toLocaleDateString()}</option>))}
                </select>
              </div>
              <div className="modal-actions">
