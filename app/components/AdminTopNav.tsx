...
export default function AdminTopNav({ variant = 'super' }: { variant?: 'super' | 'admin' }) {
  ...
  return (
    <div className="nav-links admin-nav-links" ref={menuRef} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fff' }}>
      {links.map((link) => (
        <Link key={link.href} href={link.href} className="landlord-nav-link" style={{ display: 'inline-flex', alignItems: 'center', padding: '8px 14px', borderRadius: '999px', color: '#e2e8f0', textDecoration: 'none', fontSize: '13px', fontWeight: 600, background: 'rgba(255,255,255,0.08)', transition: 'all .2s' }}>
          {link.label}
        </Link>
      ))}
      ...
      <div className="nav-menu" style={{ position: 'relative', marginLeft: '6px' }}>
        ...
        {menuOpen && (
          <div className="menu-dropdown" style={{ position: 'absolute', top: 50, right: 0, minWidth: 210, padding: 10, borderRadius: 14, background: 'linear-gradient(135deg, #0a1f3a, #061528)', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 16px 40px rgba(6,21,40,0.22)', zIndex: 100 }}>
            ...
            {links.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setMenuOpen(false)} style={{ display: 'block', padding: '10px 12px', color: '#8aa3c4', textDecoration: 'none', fontSize: '14px', fontWeight: 600, borderRadius: 8 }}>
                {link.label}
              </Link>
            ))}
