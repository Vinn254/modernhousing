...
export default function TenantsPage() {
    ...
    async function handleRemove(tenantId) {
        if (!confirm('Remove this tenant? They will be marked as relocated and the unit will be freed.'))
            return;
        ...
        <section className="card-grid">
          <article className="card" style={{ gridColumn: 'span 2' }}>
            ...
            <form onSubmit={handleSubmit} className="form-grid">
              ...
              {!editingTenant && (<select value={form.unitId} onChange={e => setForm(f => ({ ...f, unitId: e.target.value }))} required>
                  <option value="">Select unit</option>
                  {units.filter(u => u.occupancy_status === 'vacant').map(u => (<option key={u.id} value={u.id}>{u.unit_number} - {properties.find(p => p.id === u.property_id)?.name ?? '—'}</option>))}
                </select>)}
        ...
        {selectedTenant && (<section className="card-grid-item" style={{ marginTop: 24 }} ref={formRef}>
            <article className="card" style={{ gridColumn: 'span 2' }}>
              ...
              {!loadingBills && bills.length > 0 && (<div className="table-shell" style={{ maxHeight: '400px', overflowY: 'auto' }}>
                ...
                <td>
                              {bill.balance > 0 && (<button className="action-button warn" style={{ padding: '6px 10px', fontSize: '12px' }} onClick={() => handleShowPayForm(bill.id, bill.balance)}>Pay</button>)}
                            </td>
                        </tr>))}
                    </tbody>
                  </table>
