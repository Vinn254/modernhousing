...
export default function AdminDashboard() {
    ...
    const [vacantUnitsList, setVacantUnitsList] = useState([]);
    const [rentOwedByTenant, setRentOwedByTenant] = useState([]);
    ...
    useEffect(() => {
        async function loadData() {
            ...
            const mergedPayments = [...(paymentsData.payments ?? []).map((p) => ({
                    ...p,
            ...
            mergedPayments.forEach((p) => {
                if (nonPaymentTypes.includes(p.transaction_type))
                    return;
            ...
            setTotalOwed(owedTenants.reduce((sum, t) => sum + Number(t.balance_remaining || 0), 0));
            setLoading(false);
        }
        loadData();
    ...
    return (<>
      <main className="container admin-no-hero floral-bg">
        ...
        <section className="bento-grid" style={{ marginTop: 24, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          ...
          {!loading && occupiedUnits + vacantUnits > 0 && (<div className="bento-card" style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <Sparkline data={[50, 65, occupancyRate]} color="#4f46e5" w={80} h={24}/>
                <p style={{ margin: 0, color: 'var(--ink-3)', fontSize: '13px' }}>of units occupied</p>
              </div>
            </div>)}

          <button type="button" className="bento-card" style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, width: '100%', textAlign: 'left', cursor: 'pointer' }} onClick={() => setShowVacantModal(true)}>
            ...
            <div>
              ...
              <Sparkline data={[5, 3, vacantUnits]} color="#8b5cf6" w={80} h={24}/>
          ...
          <button type="button" className="bento-card" style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, width: '100%', textAlign: 'left', cursor: 'pointer' }} onClick={() => setShowRentOwedModal(true)}>
            ...
            <div>
              ...
              <Sparkline data={[0, 5000, totalOwed]} color="var(--amber)" w={80} h={24}/>
        ...
        {showVacantModal && (<div className="modal-overlay" onClick={() => setShowVacantModal(false)}>
            <div className="modal-card" style={{ maxHeight: '80vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-card-body">
                {vacantUnitsList.length > 0 ? (<div className="table-shell" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                  ...
                  </div>) : <p className="table-empty">No vacant units — every unit is occupied.</p>}
              </div>
            </div>
          </div>)}

        {showRentOwedModal && (<div className="modal-overlay" onClick={() => setShowRentOwedModal(false)}>
            <div className="modal-card" style={{ maxHeight: '80vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
              <div className="modal-card-header">
                <div>
              ...
              <div className="modal-card-body">
                {rentOwedByTenant.some(t => t.balance_remaining > 0) ? (<div className="table-shell" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                    <table className="landlord-table">
                      <thead><tr><th>Tenant</th><th>Unit</th><th>Total Paid</th><th>Balance</th><th>Last Payment</th></tr></thead>
                      <tbody>{rentOwedByTenant.filter(t => t.balance_remaining > 0).map(t => (<tr key={t.id}>
                          <td>{t.full_name}</td>
                          <td>{t.unit}</td>
                  ...
                  </div>) : <p className="table-empty">All tenants are paid up — nothing owed.</p>}
              </div>
            </div>
          </div>)}
