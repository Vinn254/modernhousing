...
export default function AuditTrailPage() {
    ...
    async function loadAuditTrails(documentId, tenantId) {
        setLoading(true);
        try {
    ...
    async function downloadAuditTrail(trail) {
        ...
        events.push({ action: 'sent', timestamp: trail.sent_at }, { action: 'viewed', timestamp: trail.viewed_at }, { action: 'signed', timestamp: trail.signed_at }, { action: 'completed', timestamp: trail.completed_at });
      ...
      <body>
        ...
        <div class="section">
          ...
          ${filteredEvents.map((e) => `<div>${e.timestamp ? new Date(e.timestamp).toUTCString() : ''} - ${e.action.charAt(0).toUpperCase() + e.action.slice(1)}</div>`).join('')}
        </div>
    ...
    if (loading) {
        return (<main className="container page-layout auth-pattern-bg">
    ...
    return (<main className="container page-layout auth-pattern-bg">
      ...
      {auditTrails.length === 0 && !error && (<div className="card">
          <p>No audit trails found.</p>
        </div>)}

      {auditTrails.length > 0 && (<div className="table-shell">
          <table className="landlord-table">
            ...
            <tbody>
              {auditTrails.map(trail => (<tr key={trail.id}>
                  ...
                  <td>
                    <button className="action-button secondary" onClick={() => downloadAuditTrail(trail)} style={{ padding: '6px 12px', fontSize: '12px' }}>
                      Download
                    </button>
                  </td>
                </tr>))}
